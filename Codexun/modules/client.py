...
async def pausevc(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            await CallbackQuery.answer(f"لايوجد شيء مشغل!", show_alert=True)
            return
    else:
        await CallbackQuery.answer(f"لايوجد شيء مشغل!", show_alert=True)
...
**قناة السورس : https://t.me/ektesa7**

**شكرا لك !""",
       reply_markup=InlineKeyboardMarkup(
            [[
                    InlineKeyboardButton("- 𝖠𝗁ٰ𝗆ᥱժ 𝖪h𝖺łꫀժ ", url=f"https://t.me/dddfx0")
                ],
...
انا بوت استطيع تشغيل الاغاني في المحادثات الصوتيه وايضا استطيع تشغيل الفيديوهات والكثير من المميزات الاخرى
...
كل الشكر والاحترام لمن قام بأضافة بوتاتنا 📍""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🌐 | السورس", callback_data="cbcmnds"),
                    InlineKeyboardButton(
                        "🖥️ | الاوامر", callback_data="https://t.me/ektesa7")
                ],
                [
                    InlineKeyboardButton(
...
async def nonabout(_, query: CallbackQuery):
    await query.edit_message_text(
        f"""**اهلا بك عزيزي في سورس برق هذه ازرار قنواتنا وجروباتنا**""",
        reply_markup=InlineKeyboardMarkup(
            [
              [
